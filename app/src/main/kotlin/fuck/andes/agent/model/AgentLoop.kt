...
internal class AgentLoop(
    ...
    private fun appendToolOutcomes(
        ...
        } else if (visionRouter != null) {
            // 主模型不支持图像输入，但配置了视觉分流模型：把截图交给视觉模型生成文字描述后回注。
            val images = imageOutcomes.flatMap { outcome -> outcome.result.images }
            val description = visionRouter.describe(images, toolNames)
            messages.put(
                AgentConversationCodec.userTextMessage(
                    "工具 $toolNames 返回了观察截图，当前模型不支持图像输入，已自动交由视觉模型处理。\n" +
                        "以下是视觉模型对截图的描述：\n$description\n\n" +
                        "请基于以上描述（而不是图片本身）继续执行任务。"
                )
            )
        } else {
