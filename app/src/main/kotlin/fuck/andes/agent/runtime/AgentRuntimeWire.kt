...
internal object AgentRuntimeWire {
    ...
    /** client -> service：向正在运行的 Agent 注入一条引导（supplement），不打断当前 run。 */
    const val MSG_SUPPLEMENT = 9
    ...
    fun supplementFromBundle(bundle: Bundle): Pair<String, String> =
        bundle.getString(KEY_RUN_ID).orEmpty() to bundle.getString(KEY_SUPPLEMENT_TEXT).orEmpty()
    ...
    fun eventToBundle(event: AgentEvent): Bundle = Bundle().apply {
        when (event) {
            ...
            is AgentEvent.RunQueued -> {
                putString(KEY_TYPE, "run_queued")
                putString(KEY_RUN_ID, event.runId)
                putInt("queue_size", event.queueSize)
            }

            is AgentEvent.RunQueueStarted -> {
                putString(KEY_TYPE, "run_queue_started")
                putString(KEY_RUN_ID, event.runId)
                putInt("queue_size", event.queueSize)
            }
    ...
    fun eventFromBundle(bundle: Bundle): AgentEvent? = when (bundle.getString(KEY_TYPE)) {
        ...
        "run_queued" -> AgentEvent.RunQueued(
            runId = bundle.getString(KEY_RUN_ID).orEmpty(),
            queueSize = bundle.getInt("queue_size"),
        )

        "run_queue_started" -> AgentEvent.RunQueueStarted(
            runId = bundle.getString(KEY_RUN_ID).orEmpty(),
            queueSize = bundle.getInt("queue_size"),
        )
