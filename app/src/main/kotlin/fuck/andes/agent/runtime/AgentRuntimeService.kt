...
internal class AgentRuntimeService : Service(), LifecycleOwner, SavedStateRegistryOwner {
    ...
    private val runQueue = ArrayDeque<QueuedRun>()
    private val maxRunQueueSize = 5
        ...
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ...
                AgentRuntimeWire.MSG_SUPPLEMENT -> {
                    ...
                    val (runId, text) = AgentRuntimeWire.supplementFromBundle(data)
                    if (runId.isNotBlank()) {
                        val session = activeSession
                        if (session != null && session.runId == runId) {
                            requestSupplement(text)
                        } else {
                            AndroidAgentLogger.debug { "Agent runtime ignored stale supplement request" }
                        }
                    }
                }
    ...
    private fun enqueueOrReject(
        ...
        if (runQueue.size >= maxRunQueueSize) {
            sendResultTo(
                replyTo,
                AgentRuntimeWire.RunResult(
                    runId = request.runId,
                    ok = false,
                    content = "",
                    error = "任务队列已满（最多 $maxRunQueueSize 个），请稍后再试",
                ),
            )
            return
        }
        runQueue.addLast(QueuedRun(request, replyTo))
        sendEventTo(replyTo, AgentEvent.RunQueued(runId = request.runId, queueSize = runQueue.size))
