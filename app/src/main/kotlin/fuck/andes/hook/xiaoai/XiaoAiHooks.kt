...
internal object XiaoAiHooks {
    ...
    private fun maybeClaimRequest(
        ...
        // Codex 式引导：同一对话已有活跃 run 且本轮无新图片时，把新语音作为引导注入当前 run，
        // 不打断正在执行的任务，让用户在 Agent 工作途中随时可以补充想法。
        val ongoingRun = activeRun.get()
        if (ongoingRun != null && ongoingRun.dialogId == dialogId && !ongoingRun.cancelled.get() && image == null) {
            val ongoingClient = ongoingRun.client.get()
            if (ongoingClient != null) {
                ongoingClient.submitSupplement(ongoingRun.runId, query)
                claimedDialogIds.add(dialogId)
                logger.info(
                    "超级小爱续说已作为引导注入当前 run: runId=${shortId(ongoingRun.runId)}, " +
                        "queryChars=${query.length}"
                )
                return true
            }
